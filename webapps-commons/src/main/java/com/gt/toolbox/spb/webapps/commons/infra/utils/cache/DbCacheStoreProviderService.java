...
/**
 * Defines the contract for a service that provides a persistent cache store, typically backed by a
 * database. This interface abstracts the underlying data access mechanism for cache operations.
 *
...
public interface DbCacheStoreProviderService<K, V> {
    ...
    /**
     * Builds a new cache entity instance from a key and a value. The entity must also track its
     * last access time.
     *
     * @param <T> the type of the entity, which must extend {@link KeyValueDto} and
     *        {@link GtLastAccessed}
     * @param key the key for the cache entry.
     * @param value the value for the cache entry.
     * @return a new entity instance, not yet persisted.
     */
    <T extends KeyValueDto<K, V> & GtLastAccessed> T buildEntity(K key, V value);
     ...
     * @return the saved entity, which may have been updated by the persistence mechanism (e.g.,
     *         with a generated ID).
     */
    <T extends KeyValueDto<K, V> & GtLastAccessed> T save(T entity);
     ...
     * @return an {@link Optional} containing the found entity, or {@link Optional#empty()} if no
     *         entity is found for the given key.
     */
    <T extends KeyValueDto<K, V> & GtLastAccessed> Optional<T> findByKey(K key);

    /**
     * Checks if the cache contains an entry for the specified key.
     *
     * @param key the key whose presence in this cache is to be tested.
     * @return {@code true} if this cache contains a mapping for the specified key, {@code false}
     *         otherwise.
     */
